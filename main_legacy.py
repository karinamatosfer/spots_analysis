...
class Fish:
    ...
    def moveoriginalimage(self):
        
        assert "volumes" in self.dataPaths, "must contain image volumes"

        for key in tqdm(self.dataPaths["volumes"].keys(), "moving orig_img in plane: "):
            imgpath = self.dataPaths["volumes"][key]["image"].parents[0].joinpath("image.tif")
    ...
    def pandastim_to_df(pstimpath):
        ...
        mini_stim = stimulus_df.loc[:, ['stim_name', 'static_time',
